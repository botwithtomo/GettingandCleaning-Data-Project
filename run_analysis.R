...
# CAUTION! This is for the group
# this one is pretty cool tho, I did this for my personal interests
...
groupdata= rbind(activity_mean,subject_mean)

# CAUTION This is what you want!
# get independent tidy data set with the average of each variable for each activity and each subject.


data=NULL
nameactivity = c("WALKING","WALKING_UPSTAIRS", "WALKING_DOWNSTAIRS", "SITTING", "STANDING","LAYING")
for (i in 1:6){
	for (m in 1:30){
		temp = sapply(total[subject==m & activity ==nameactivity[i],-c(1,2),with=FALSE],mean)
		temp = c(m,nameactivity[i],temp)
...
write.table(total,row.names=FALSE,file="rawdata.txt")
write.table(groupdata,row.names=FALSE,file="groupdata.txt")
write.table(data,row.names=FALSE,file="tidydata.txt")
